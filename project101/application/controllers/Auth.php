...
class Auth extends CI_Controller {
    ...
    public function chk_login(){
        ...
        if( $this->input->post('email') !== FALSE ){
            ...
            $ret = $this->_check_user($email,$passwd);
            if( $ret === TRUE){
    ...
    private function _check_user($email,$passwd){

        $rs = $this->db->query("select * from users where user_email = ? and user_password = ? ",
            array($email,$passwd)
        );

        if ( $rs->num_rows() !== 0 ){

            $this->_check_user_type($rs->row(0)->user_type);

            $this->session->set_userdata('is_owner',$this->is_owner);
            $this->session->set_userdata('is_customer',$this->is_customer);
            $this->session->set_userdata('is_beautician',$this->is_beauticians);

            $this->session->set_userdata('user_id',$rs->row(0)->user_id);
            $this->session->set_userdata('user_firstname',$rs->row(0)->user_firstname);
            $this->session->set_userdata('user_lastname',$rs->row(0)->user_lastname);
            $this->session->set_userdata('user_gender',$rs->row(0)->user_gender);
            $this->session->set_userdata('user_email',$rs->row(0)->user_email);
            $this->session->set_userdata('user_phone',$rs->row(0)->user_phone);
            $this->session->set_userdata('user_position',$rs->row(0)->user_type);
            $this->session->set_userdata('user_point',$rs->row(0)->user_point);
            return TRUE;
        }
    ...
    private function _check_user_type($type=NULL)
    {
        if($type == "customer"){
            $this->is_customer = TRUE;
        }elseif($type == "beautician"){
            $this->is_beauticians = TRUE;
        }elseif($type =="owner"){
            $this->is_owner = TRUE;
        }else{
            $this->session->set_userdata('logged_in',FALSE);
